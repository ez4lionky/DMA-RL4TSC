...
class Intersection:
    ...
    def update_current_measurements(self, simulator_state):
        ...
        if self.list_lane_vehicle_previous_step_in:  # previous step is not empty
            list_vehicle_new_left = list(set(self.list_lane_vehicle_previous_step_in) -
                                         set(self.list_lane_vehicle_current_step_in))
...
class CityFlowEnv:
    ...
    def batch_log_2(self, logger=None):
        """
        ...
        for inter_ind in range(self.dic_traffic_env_conf["NUM_INTERSECTIONS"]):
            ...
            df_vehicle_inter = pd.read_csv(path_to_log_file, sep=',', header=0, dtype={0: str, 1: float, 2: float},
            ...
            # if leave time is NaN, then travel time will still be zero
        ...
        tmp_idx = []
        for i, v in enumerate(df_vehicle_all["vehicle_id"]):
            if "shadow" in v:
                tmp_idx.append(i)
        df_vehicle_all.drop(df_vehicle_all.index[tmp_idx], inplace=True)
        # calculate the duration through the entire network
        # nan_ids = df_vehicle_all[df_vehicle_all.isna().any(axis=1)]['vehicle_id']
        # val_df_vehicle_all = df_vehicle_all.loc[~df_vehicle_all['vehicle_id'].isin(nan_ids)]
        val_df_vehicle_all = df_vehicle_all
        vehicle_duration = val_df_vehicle_all.groupby(by=['vehicle_id'])['duration'].sum()
        ave_duration = vehicle_duration.mean()  # mean amomng all the vehicle
        if logger:
            logger.info(f"Recalculated average travel time: {ave_duration}")

...
def test(model_dir, cnt_round, run_cnt, _dic_traffic_env_conf, logger=None):
    ...
    try:
        ...
        if logger:
            logger.info(f"episode:{cnt_round}/250, steps:{run_cnt}")
            logger.info(f"Average Travel Time: {env.eng.get_average_travel_time()}")
        env.batch_log_2(logger)
        env.end_cityflow()
    except:

...
parser.add_argument('--save_dir', type=str, default="model/dqn_ma_gat", help='directory in which model should be saved')
...
parser.add_argument('--with_speed_score', type=u.str2bool, const=True, nargs='?',
                    default=False, help='If reward combines the speed score')
...
# create agents
agents = []
reward_fns = ["lane_waiting_count", "speed_score"] if args.with_speed_score else ["lane_waiting_count"]
for i in world.intersections:
    ...
    agents.append(DQNAgent(
        ...
        LaneVehicleGenerator(world, i, reward_fns, in_only=True, average="all", negative=True),
    ...
    for e in range(episodes):
        ...
        while i < args.steps:  # 3600 steps
            if i % action_interval == 0:
                ...
                # mean rewards in an action interval is better, for example, if we
                ...
                for agent_id, agent in enumerate(agents):
                    agent.remember(last_hist_obs[agent_id], last_obs_phase[agent_id], last_obs_map[agent_id],
                    ...
                    if first_replay:
                        for epoch in tqdm(range(epochs_initial_replay)):
                            ...
                            if args.model_average != 'None':
                                weight_matrix = u.average_weights_matrix(args, len(agents), world, device)

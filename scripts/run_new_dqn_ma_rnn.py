...
parser.add_argument('--network', type=str, default="RNN", choices=("FC", "CNN", "RNN", "Full"),
...
def train(args, env):
    ...
    for e in range(episodes):
        ...
        last_hist_obs = np.array(history_buffer)
            ...
            if i % action_interval == 0:
                ...
                for agent_id, agent in enumerate(agents):
                    if total_decision_num > agent.learning_start:
                        action, _ = agent.get_action(last_hist_obs[agent_id], last_obs_phase[agent_id],
                                                     last_obs_map[agent_id], edges_list[agent_id],
                                                     last_edge_features[agent_id])
                    else:
                        action, _ = agent.sample(last_hist_obs[agent_id])
                ...
                hist_obs = np.array(history_buffer)
